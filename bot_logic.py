...
# Comandos de sala - aceita de qualquer mensagem (selfbot nao dispara on_message para si mesmo)
        if cmd.startswith(".v ") and message.author == client.user:
            ...
            log_msg(user_id, f"💰 .v | Nome: {nome_busca}")
            msg_aguarda = await _digitar_e_enviar(channel, f"⏳ Verificando pagamento de `{nome_busca}`...")
            try:
            ...
            if resultado:
                ...
                log_msg(user_id, f"💰 Progresso: {pagamentos_por_thread[channel.id]}/2")
